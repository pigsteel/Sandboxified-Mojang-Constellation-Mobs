...
public class smcm$LayerDefinitions {
    ...
    public static void registerModelLayers(IClientRegistryHelper registrar) {
        ...
        registrar.registerModelLayer(smcm$ModelLayers.RECLAIMED, () -> ReclaimedModel.createBodyLayer(CubeDeformation.NONE).apply(reclaimedScale));
        registrar.registerModelLayer(smcm$ModelLayers.RECLAIMED_BABY, () -> BabyReclaimedModel.createBodyLayer(CubeDeformation.NONE));
        registerArmorLayers(smcm$ModelLayers.RECLAIMED_ARMOR, humanoidArmor.map(layer -> layer.apply(reclaimedScale)), registrar);
        registerArmorLayers(smcm$ModelLayers.RECLAIMED_BABY_ARMOR, humanoidBabyArmor, registrar);
        registrar.registerModelLayer(smcm$ModelLayers.RECLAIMED_OUTER_LAYER, () -> ReclaimedModel.createBodyLayer(new CubeDeformation(0.25F)).apply(reclaimedScale));
        ...
        registrar.registerModelLayer(smcm$ModelLayers.ENCHANTER, () -> EnchanterModel.createBodyLayer().apply(ILLAGER_SCALE));
        ...
        registrar.registerModelLayer(smcm$ModelLayers.NECROMANCER, () -> NecromancerModel.createBodyLayer().apply(MeshTransformer.scaling(1.2F)));
        registrar.registerModelLayer(smcm$ModelLayers.NECROMANCER_CLOAK, () -> NecromancerCloakModel.createCloakLayer().apply(MeshTransformer.scaling(1.2F)));
    }
