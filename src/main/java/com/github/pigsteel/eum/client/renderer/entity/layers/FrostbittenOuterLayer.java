...
import com.github.pigsteel.eum.client.renderer.entity.FrostbittenRenderer;
...
public class FrostbittenOuterLayer extends RenderLayer<FrostbittenRenderState, FrostbittenModel> {
    ...
    public FrostbittenOuterLayer(RenderLayerParent<FrostbittenRenderState, FrostbittenModel> renderer, final EntityModelSet modelSet) {
        super(renderer);
        this.model = new FrostbittenModel(modelSet.bakeLayer(EUMModelLayers.FROSTBITTEN_OUTER_LAYER));
        this.babyModel = new BabyFrostbittenModel(modelSet.bakeLayer(EUMModelLayers.FROSTBITTEN_BABY_OUTER_LAYER));
    }
