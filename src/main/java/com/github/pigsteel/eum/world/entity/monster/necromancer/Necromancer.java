package com.github.pigsteel.eum.world.entity.monster.necromancer;
...
public class Necromancer extends Monster {
    ...
    private void spawnSummoningOrbParticles() {
        ...
        this.level().addParticle(
				EUMParticleTypes.NECROMANCER_MAGIC.get(),
    ...
    protected SoundEvent getAmbientSound() {
        return this.random.nextFloat() < 0.05F
                ? EUMSoundEvents.NECROMANCER_LAUGH.get()
                : EUMSoundEvents.NECROMANCER_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(final DamageSource source) {
        return EUMSoundEvents.NECROMANCER_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return EUMSoundEvents.NECROMANCER_DEATH.get();
    }

    protected SoundEvent getStepSound() {
        return EUMSoundEvents.NECROMANCER_STEP.get();
    }

	public SoundEvent getPrepareSummonSound() {
		Component customName = this.getCustomName();
		if(customName != null && customName.getString().equals("Sandy")) {
			return EUMSoundEvents.NECROMANCER_PREPARE_SUMMON_ALT.get();
		} else {
			return EUMSoundEvents.NECROMANCER_PREPARE_SUMMON.get();
		}
	}
	...
	static {
		BRAIN_PROVIDER = Brain.provider(
				List.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.HURT_BY, SensorType.NEAREST_PLAYERS, EUMSensorTypes.NECROMANCER_ATTACK_ENTITY_SENSOR.get()),
				NecromancerAi::getActivities
		);
		DATA_SPELL = SynchedEntityData.defineId(Necromancer.class, EUMEntityDataSerializers.NECROMANCER_SPELL);
	}
