...
import com.github.pigsteel.eum.world.entity.ai.behavior.necromancer.ShootingMagic;
import com.github.pigsteel.eum.world.entity.ai.behavior.necromancer.Summoning;
	...
	static class ApproachOrGlareIfCannotAttack {
		...
		public static BehaviorControl<Mob> create(final Function<LivingEntity, Float> speedModifier) {
			return BehaviorBuilder.create((i) -> i.group(
					...
					i.present(EUMMemoryModuleTypes.SUMMONING_COOLDOWN.get()),
					i.present(EUMMemoryModuleTypes.SHOOTING_COOLDOWN.get()),
					i.absent(EUMMemoryModuleTypes.PENDING_SUMMON.get())
			).apply(i, (
					walkTarget,
					lookTarget,
					attackTarget,
