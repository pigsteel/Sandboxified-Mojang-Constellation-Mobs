...
public class Sunken extends AbstractSkeleton implements CrossbowAttackMob, Shearable {
    ...
    protected void readAdditionalSaveData(final ValueInput input) {
        ...
        VariantUtils.readVariant(input, EUMCustomRegistries.SUNKEN_VARIANT).ifPresent(this::setVariant);
    ...
    public SoundEvent getAmbientSound() {
        return (this.isInWater() ? EUMSoundEvents.SUNKEN_AMBIENT_WATER : EUMSoundEvents.SUNKEN_AMBIENT).get();
    }

    @Override
    public SoundEvent getHurtSound(final DamageSource source) {
        return (this.isInWater() ? EUMSoundEvents.SUNKEN_HURT_WATER : EUMSoundEvents.SUNKEN_HURT).get();
    }

    @Override
    public SoundEvent getDeathSound() {
		return (this.isInWater() ? EUMSoundEvents.SUNKEN_DEATH_WATER : EUMSoundEvents.SUNKEN_DEATH).get();
    }

    public SoundEvent getStepSound() {
		return (this.isInWater() ? EUMSoundEvents.SUNKEN_STEP_WATER : EUMSoundEvents.SUNKEN_STEP).get();
    }

    @Override
    public void shear(ServerLevel level, SoundSource soundSource, ItemStack tool) {
		level.playSound(null, this, EUMSoundEvents.SUNKEN_SHEAR.get(), soundSource, 1.0F, 1.0F);

		this.dropFromShearingLootTable(
				level,
				EUMLootTables.SHEAR_SUNKEN,
				tool,
    ...
    static {
		DATA_VARIANT_ID = SynchedEntityData.defineId(Sunken.class, EUMEntityDataSerializers.SUNKEN_VARIANT);
    ...
    protected void applyImplicitComponents(final DataComponentGetter components) {
        this.applyImplicitComponentIfPresent(components, EUMDataComponents.SUNKEN_VARIANT.get());
		this.applyImplicitComponentIfPresent(components, EUMDataComponents.IS_CORAL_DEAD.get());
    }


	@Override
	public <T> @Nullable T get(final DataComponentType<? extends T> type) {
		if (type == EUMDataComponents.SUNKEN_VARIANT.get()) {
			return (T) castComponentValue(EUMDataComponents.SUNKEN_VARIANT.get(), this.getVariant());
		} else if (type == EUMDataComponents.IS_CORAL_DEAD.get()) {
			return (T) castComponentValue(EUMDataComponents.IS_CORAL_DEAD.get(), this.isCoralDead());
		}
	...
	protected <T> boolean applyImplicitComponent(final DataComponentType<T> type, final T value) {
		if (type == EUMDataComponents.SUNKEN_VARIANT.get()) {
			this.setVariant(castComponentValue(EUMDataComponents.SUNKEN_VARIANT.get(), value));
			return true;
		} else if (type == EUMDataComponents.IS_CORAL_DEAD.get()) {
			this.setIsCoralDead(castComponentValue(EUMDataComponents.IS_CORAL_DEAD.get(), value));
			return true;
		} else {
