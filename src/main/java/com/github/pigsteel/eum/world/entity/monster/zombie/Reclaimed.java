...
import com.github.pigsteel.eum.world.entity.Bonemealable;
...
public class Reclaimed extends Zombie implements Bonemealable, Shearable, RangedAttackMob {
    ...
    public <T> @Nullable T get(final DataComponentType<? extends T> type) {
        if (type == EUMDataComponents.RECLAIMED_HEAD_FLOWER.get()) {
            HeadFlower flower = this.getHeadFlowerType();

            return flower == null
                    ? null
                    : (T) castComponentValue(EUMDataComponents.RECLAIMED_HEAD_FLOWER.get(), flower);
        }
    ...
    protected SoundEvent getAmbientSound() {
        return EUMSoundEvents.RECLAIMED_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(final DamageSource source) {
        return EUMSoundEvents.RECLAIMED_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return EUMSoundEvents.RECLAIMED_DEATH.get();
    }

    @Override
    protected SoundEvent getStepSound() {
        return EUMSoundEvents.RECLAIMED_STEP.get();
    }
    ...
    public void bonemeal(ServerLevel level, SoundSource soundSource, ItemStack bonemealItem) {
        level.playSound(null, this, EUMSoundEvents.RECLAIMED_BONEMEAL.get(), soundSource, 1.0F, 1.0F);
        this.setHeadFlower(
    ...
    public void shear(ServerLevel level, SoundSource soundSource, ItemStack tool) {
        level.playSound(null, this, EUMSoundEvents.RECLAIMED_SHEAR.get(), soundSource, 1.0F, 1.0F);
        // check the flower and then take it off ()
        this.dropFromShearingLootTable(
                level,
                EUMLootTables.SHEAR_RECLAIMED,
                tool,
    ...
    protected void applyImplicitComponents(final DataComponentGetter components) {
        this.applyImplicitComponentIfPresent(components, EUMDataComponents.RECLAIMED_HEAD_FLOWER.get());
    ...
    protected <T> boolean applyImplicitComponent(final DataComponentType<T> type, final T value) {
        if (type == EUMDataComponents.RECLAIMED_HEAD_FLOWER.get()) {
            this.setHeadFlower(castComponentValue(EUMDataComponents.RECLAIMED_HEAD_FLOWER.get(), value).defaultBlockState());
            return true;
        } else {
