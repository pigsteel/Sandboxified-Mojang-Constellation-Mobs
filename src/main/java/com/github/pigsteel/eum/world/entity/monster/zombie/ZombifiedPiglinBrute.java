...
public class ZombifiedPiglinBrute extends ZombifiedPiglin {
	...
	protected SoundEvent getAmbientSound() {
		return (this.isAngry() ? EUMSoundEvents.ZOMBIFIED_PIGLIN_BRUTE_ANGRY : EUMSoundEvents.ZOMBIFIED_PIGLIN_BRUTE_AMBIENT).get();
	}

	protected SoundEvent getHurtSound(final DamageSource source) {
		return EUMSoundEvents.ZOMBIFIED_PIGLIN_BRUTE_HURT.get();
	}

	protected SoundEvent getDeathSound() {
		return EUMSoundEvents.ZOMBIFIED_PIGLIN_BRUTE_DEATH.get();
	}

	@Override
	public void playAngerSound() {
		this.playSound(EUMSoundEvents.ZOMBIFIED_PIGLIN_BRUTE_ANGRY.get(), this.getSoundVolume() * 2.0F, this.getVoicePitch() * 1.8F);
	}
}
