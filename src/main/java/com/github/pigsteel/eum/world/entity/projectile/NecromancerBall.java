...
public class NecromancerBall extends Projectile {
	...
	public <E extends Necromancer> NecromancerBall(E necromancer, Level level) {
		this(EUMEntityTypes.NECROMANCER_BALL.get(), level);
	...
	protected void onHitEntity(final EntityHitResult hitResult) {
		...
		if (level instanceof ServerLevel serverLevel) {
			...
			if (owner instanceof LivingEntity livingOwner) {
				...
				if (wasHurt) {
					this.playSound(EUMSoundEvents.NECROMANCER_BALL_HIT.get(), 1.0F, 1.0F);
					EnchantmentHelper.doPostAttackEffects(serverLevel, entity, damageSource);
					if (entity instanceof LivingEntity livingEntity) {
						if(livingEntity.hasEffect(EUMMobEffects.CORRUPTION)) {
							livingEntity.addEffect(new MobEffectInstance(EUMMobEffects.CORRUPTION, 60 * 20, livingEntity.getEffect(EUMMobEffects.CORRUPTION).getAmplifier() + 1));
						} else {
							livingEntity.addEffect(new MobEffectInstance(EUMMobEffects.CORRUPTION, 60 * 20, 0));
						}
					}
				}
			}
	...
	private void createParticleTrail() {
		...
		for(int i = 0; i < 3; i++) {
			...
			this.level().addParticle(EUMParticleTypes.NECROMANCER_MAGIC.get(), xx, yx, zx, 0.0F, 0.0F, 0.0F);
		}
	}
